...
(function($) {  
    $.addGt = function(t, p) {   
        ...
        p = $.extend({
            ...
            title: '', 
            exchangeColor: true,
            closeImg: '/images/tabletree/minus.gif',
            openImg: '/images/tabletree/plus.gif',
            lazyLoadImg: '/images/tabletree/plus2.gif',
            blankImg: '/images/tabletree/blank.gif',
            noparentImg: '/images/tabletree/tv-item.gif',
            lastLeafImg: '/images/tabletree/tv-item-last.gif',
            morePageImg: '/images/tabletree/morePage.gif',
            dirImg: '/images/tabletree/folder.gif',
            fileImg: '/images/tabletree/file.gif',
        ...
        var g = {
			...
			/*******************************************************************
			 * make table first point the process reaches
			 * 
			 * @param data
			 */
            _makeTable: function(data) {
               ...
               /***************************************************************
    			...
    			//g._open('1');
                if (p.pageSize) {
                	//alert("p.pageSize is not null!"+p.pageSize);
			...
			*/
            ...
            _analyseData: function(msg) {
                ...
                if (_serverPagingMode == 'server') {
                    ...
                    if (_serverPagingMode == 'client') {
						...
						p.allDataInfoWithPageInfo=msg;
                        data = p.allDataInfoWithPageInfo.data;
                        if(typeof p.allDataInfoWithPageInfo.page !='undefined'){
                        	pagingInfo.currentPage = p.allDataInfoWithPageInfo.page;
                        }
                        if(typeof p.allDataInfoWithPageInfo.total!='undefined'){
            ...
            _reload: function() { 
                ...
                $.ajax({
                    ...
                    success: function(msg) {
                        ...
                        g._showMsg(0);
                        g._setLastLeaf();
                        mcel.innerHTML = ["Page ", pagingInfo.currentPage, "of ", pagingInfo.pagesCount, " "].join('');
                        var gotime = new Date() - o;
            ...
            dynamicAdd: function(parentId, parentNode, imgNode, childNodeStrs, prebotherindex) { 
                ...
                if (!(p.lazyPage || p.lazyMorePage)) { 
                   ...
                   // datas = p.repaintDataInfoDatas;
                    ...
                    //} 
					//p.allDataInfoWithPageInfo=msg;
					//p.repaintDataInfoDatas=childNodeStrs.data;
					if(childNodeStrs.data){
						//alert("data.");
						datas=childNodeStrs.data;
					}
                ...
                for (var i = 0; i <= datas.length - 1; i++) { 
                    ...
                    if(p.lazyPage||p.lazyMorePage){
                        ...
                        if (p.lazyPage && (lazypaging_info.allCount >= lazypaging_info.pageSize) && (i == datas.length - 1) && !p._dynamicAddByUse) {
                        ...
                        } else if (p.lazyMorePage && (lazypaging_info.allCount >= lazypaging_info.pageSize) && (i == datas.length - 1) && !p._dynamicAddByUse) {
            ...
            dynamicAddSingleChild:function(parentId, parentNode, imgNode, childNodeStrs, prebotherindex){
            	if (!imgNode) { 
                    g._e('动态添加表格行出现错误,可能是没有找到pId对应的img！');
                    return false;
                ...
                //imgNode.src = p.closeImg;
                /******************************/
                if($(imgNode).attr('src').indexOf(p.openImg)>-1){
                	$(imgNode).unbind('click').bind('click',function(e) { 
                		g.closeChildrenTable('img_' + parentId,null,e) ;
                	});
                }
                ...
                for (var i = 0; i <= datas.length - 1; i++) {
                    ...
                    var newRow = g._addOneLazyRowByData(startIndex, datas[i], level, 1 + prebotherindex * 1, null);   
                ...
                if (p.lazyPage || (p.lazyMorePage && p._dynamicAddByUse)) {
                	...
                	if($(parentNode).attr('_expaned')=='true'){
                    	if($("tr[_node_path$='_node" + parentId + "']").size()>0){
                    		//alert("ifififiifif");
                        	var lastChild = $("tr[_node_path$='_node" + parentId + "']:last");
                        	var lastChildNodeId = $(lastChild).attr('id'); 
                        	if($("tr[_node_path*='" + lastChildNodeId + ",']").size()>0){
                        		$("tr[_node_path*='" + lastChildNodeId + ",']:last").after(temp);
                        	}else if($("tr[_node_path$='" + lastChildNodeId + "']").size()>0){
                        		$("tr[_node_path$='" + lastChildNodeId + "']:last").after(temp);
                        	}else
                        		$("tr[_node_path$='_node" + parentId + "']:last").after(temp);
                        }
                    	else{
                        	$(parentNode).after(temp);
                        }
                    	$("tr[_node_parent='_node"+parentId+"']").css("display","table-row");
                    	$("tr[_node_path='_node"+parentId+",']").css("display","table-row");
                    } else 
                    	{
                        var ppId =  $(parentNode).attr('_node_parent');
                        var pId = '_node'+parentId;
						var leafChildren=[]; 
						leafChildren= $('tr[_node_parent='+ppId+'][_node_isparent=0]');
						var length=leafChildren.length;
						if(length>1){
                    	if($(leafChildren[length-1]).attr('id')==pId){

        								var lastChild=leafChildren[length-2];
        								var lastChildId = $(lastChild).attr('id').replace('_node','');

        								if(lastChildId){
        									var level = $('tr[id='+pId+']').attr('_node_level');
        									var lastLevelMapKey = 'level'+level+ppId.replace('_node','');
        				                	lastLeafMap.put(lastLevelMapKey, lastChildId);
        								}
        							}
        							
        					}
                    	/***************************************************/
						$(parentNode).attr('_node_isparent','1');
                    		g._open(parentId);
                    	}
                }
                imgNode.src = p.closeImg;
                $(imgNode).bind('click',function(e) { 
            		g.closeChildrenTable('img_' + parentId,null,e) ;
            	});
                if (p.exchangeColor) g._setColor();
                p.repaintDataInfoDatas = null;
			...
			_deleteByPath:function(ppth,e){
				$("tr[_node_path^='" + ppth + "']",elct).each(function(){
					var $this = $(this);
					$this.remove();
				});
			},
			_deleteById:function(dId){
				var id = "_node"+dId;
				var tr = $("tr[id=_node"+dId+"]");
				var pId = $(tr).attr("_node_parent");

				//the node to be deleted is the only child of its parent
				if($("tr[_node_parent="+pId+"]").size()==1){
					$("tr[id="+pId+"]").attr("_node_isparent","0");
					$("tr[id="+pId+"]").attr("_expaned","false");
					
					var ppId = $("tr[id="+pId+"]").attr("_node_parent");
					var leafChildren=[]; 
					leafChildren= $("tr[_node_parent="+ppId+"][_node_isparent=0]");
					var length=leafChildren.length;
					if(length>0){	
					
					//parent is the last leaf child of grandparent
					if($(leafChildren[length-1]).attr("id")==pId){
						var level = $("tr[id="+pId+"]").attr("_node_level");
						var lastLevelMapKey = "level"+level+ppId.replace("_node","");
						if(lastLeafMap.get(lastLevelMapKey)!=null){
	                		var ddId = lastLeafMap.get(lastLevelMapKey);
	                    	var _lastLeafId = "img_" + ddId;
	                    	if($("#"+_lastLeafId)){
	                    		var src = $("#" + _lastLeafId).attr("src");
	                    		if(src){
	                    			if(src.indexOf(p.lastLeafImg)!=-1){
	                    				$("#" + _lastLeafId).attr("src", p.noparentImg);
	                    			}
	                    		}
	                    	}
	                	}
	                	lastLeafMap.put("level" + level + ppId.replace("_node",""), pId.replace("_node",""));
	                	$("#img_"+pId.replace("_node","")).attr("src",p.lastLeafImg).unbind("click");
					}
					else{
						$("#img_"+pId.replace("_node","")).attr("src",p.noparentImg).unbind("click");
					}
				}
					...
					var leafChildren=[]; 
					leafChildren= $("tr[_node_parent="+pId+"][_node_isparent=0]");
					var length=leafChildren.length;
					// last leaf child of the parent
					if(length>1&&$(leafChildren[length-1]).attr("id")==id){
						var lastChild=leafChildren[length-2];
						var lastChildId = $(lastChild).attr("id").replace("_node","");
						if(lastChildId){
							var level = $("tr[id="+id+"]").attr("_node_level");
							var lastLevelMapKey = "level"+level+pId.replace("_node","");
							lastLeafMap.put(lastLevelMapKey, lastChildId);
						}	
					}
				}
            ...
            _postGetTable: function(u, params) {
                $.ajax({
                    type: "POST",
                    url: u,
                    async: true,
            ...
            _setLastLeaf: function() {
                if (lastLeafMap.size() > 0) {
                    ...
                    for (__i = 0; __i < allLeaf.length; __i++) {
                    	var _lastLeafId = 'img_' + (allLeaf[__i]);
						var src = $('#' + _lastLeafId).attr('src');
						if(src){
			 ...
			 * 
			 * @param tableTree
			 * @param startParentIndex
			 * @param endParentIndex
			 */
            _showTable: function(tableTree, startParentIndex, endParentIndex) { 
            	g._clearContent(); 
                var rowCount = 1;  
                /***************************************************************
				 * first show the firstLevel nodes
				 */
                var lenlen = firstLevelNodes.length;
				 for (var ind = startParentIndex; ind < endParentIndex && ind < lenlen; ind++) {
                    ...
                    rowCount = rowCount + 1;
                    /***********************************************************
					 * what is the meaning of level 2?
					 */
...
//                        node.innerHTML = texts.replace(/[$]/g, value); 
            ...
            _showLazyPagingDiv: function(pid, imgId, v, way) { 
               ...
               if (v) {
                    ...
                    if (way == 1) {
                        ...
                        lazyPageDiv.style.top = offset.top- height;
                        $('#_lazy_page_currentpage_' + pid).text(lastcount);

                    }
                    lazyPageDiv.style.left = offset.left+width;
                    lazypagex = offset.left+width;
                    lazypagex2 = offset.left +width+ $(lazyPageDiv).width();
            ...
            _createLazyPagingDiv: function(pid) {
                ...
                $(lazyPagingDiv).attr('id', '_lazyPagingDiv_' + pid).addClass("lazyPagediv").html(lzpagetabletemp.replace(/\$/g, pid));              
                $(lazyPagingDiv).attr('id', '_lazyPagingDiv_' + pid).mouseout(function(e) {
                ...
                $('#_fstbtn_'+pid).bind('click',function() {
                    g._toLazyPage(pid,'first');
               });
                $('#_prebtn_'+pid).bind('click',function() {
                	g._toLazyPage(pid,'pre');
               });
                $('#_nxtbtn_'+pid).bind('click',function() {
                	g._toLazyPage(pid,'next');;
               });
                $('#_lstbtn_'+pid).bind('click',function() {
                	g._toLazyPage(pid,'last');
               });
            },
            ...
            _initLazyPageInfo: function(pid, pName, msg) {
            	...
            	p.allDataInfoWithLazyPageInfo= msg;
            	var lazypgInfo = {};
            ...
            _makePageBar: function(div) {
                ...
                $(msgCell).attr('id', '_pagebar_msgCell').addClass('pages').width(280).html(['<span><select ', ' id="_changePageSizeSel"><option value="5">5</option><option value="10" selected>10</option>', '<option value="20">20</option><option value="40">40</option><option value="100">100</option></select></span><span>Items per page&nbsp;&nbsp;&nbsp;&nbsp;</span>', "<span id='msgCell'>Page&nbsp;&nbsp;&nbsp;&nbsp;", pagingInfo.currentPage, "&nbsp;&nbsp;&nbsp;&nbsp;of&nbsp;&nbsp;&nbsp;&nbsp;", pagingInfo.pagesCount, "</span>"].join('')).appendTo(div);
                var pagesDiv = document.createElement("div");
                $(pagesDiv).attr('id', '_pagebar_pagesdiv').addClass('pagination2').html('<ul> <li class="j-first disabled"> <a style="display: none;" class="first pgb" href="javascript:;"><span>First</span></a> <span class="first"><span>First</span></span> </li> <li class="j-prev disabled"> <a style="display: none;" class="previous pgb" href="javascript:;"><span>Previous</span></a> <span class="previous"><span>Previous</span></span> </li> <li class="j-next"> <a style="display: block;" class="next pgb" href="javascript:;"><span>Next</span></a> <span style="display: none;" class="next"><span>Next</span></span> </li> <li class="j-last"> <a style="display: block;" class="pgb last" href="javascript:;"><span>Last</span></a> <span style="display: none;" class="last"><span>Last</span></span> </li> <li class="jumpto"><input class="textInput" size="4" value="1" type="text"><input class="goto"  type="button"></li> </ul>'); 
            ...
            _reMakeTable: function(pageSize) {
                if (pagingInfo.pagesCount != 0) {
                    if (_serverPagingMode == 'client') {
                        ...
                        pagingInfo.pagesCount = Math.ceil(pagingInfo.allCount / pagingInfo.pageSize * 1.0);
                        pagingInfo.currentPage = 1;
                        _$('msgCell').innerHTML = "Page&nbsp;&nbsp;&nbsp;&nbsp;" + pagingInfo.currentPage + "&nbsp;&nbsp;&nbsp;&nbsp;of&nbsp;&nbsp;&nbsp;&nbsp;" + pagingInfo.pagesCount + " ";
                        g._toPage(1);
                        _$(p.tableId).focus();
            ...
            _addOneLazyRowByData: function(index, rowData, level, nth, sp, _num) {
                ...
                if (_isP != '1'){
                	var lastLevelMapKey = 'level'+level+_parent;
                	if(lastLeafMap.get(lastLevelMapKey)!=null){
                		var dId = lastLeafMap.get(lastLevelMapKey);
                    	var _lastLeafId = 'img_' + dId;
                    	if($('#'+_lastLeafId)){
                    		var src = $('#' + _lastLeafId).attr('src');
                    		if(src){
                    			if(src.indexOf(p.lastLeafImg)!=-1){
                ...
                g._addOneLazyRowContent(rowData, newRow, _id, _isP, _isFile, level, checkSpan, sp,countV);
            ...
            _addOneLazyRowContent: function(rowData, newRow, _id, _isP,_isFile, level, checkSpan, sp,countV) {
                var i = 0;
				// 如果要添加序列所在的单独列.
				if(p._countColumnIndex==-2){
					var ctCell = $('<td></td>').css('padding','0').width(p.countCellWd);
                ...
                for (; i < headColumns.length; i++) { 
					...
					if (_t != ''){
						if(headColumns[i]=='fileName'){
							var fileImg = document.createElement("img");
							$(fileImg).css('padding-right','10px'); 
							if(_isFile==1)
								$(fileImg).attr('src',p.fileImg);
							else
								$(fileImg).attr('src',p.dirImg);
							cdiv.append(fileImg);
						}							
						var showTypeDesc = p.columnModel[i].columntype;
						if (showTypeDesc != null) {
            ...
            _createOpenImg: function(id) {
                var _i = $(["<IMG id='img_", id, "' style='CURSOR: pointer' ", " ' src='", p.openImg, "'/>"].join(''));
				_i.unbind('click');
            ...
            _createCloseImg: function(id) {
                return $(["<IMG id='img_", id, "' style='CURSOR: pointer' ", " ' src='", p.closeImg, "'/>"].join(''))
                .click(function(e) {
            ...
            _getCurrentPage: function(pginfo, pageNumId, operCode) {
                ...
                switch (operCode) {
					...
					//alert("pagesCount:"+pginfo.pagesCount); //error
                    if (pnum) {
                        var n = pnum.value;
                        var reg = /^\d+$/;
			...
			_addSingleChild:function(pid,data){
				var pNode = _$('_node'+pid);
				var img = $('#img_'+pid)[0];
				p._dynamicAddByUse = 1;
				g.dynamicAddSingleChild(pid,pNode,img,data);
				g._setLastLeaf();
			},
            ...
            _toPage: function(operCode) { 
                ...
                tb = _$(p.tableId);
                var temp = 0;
				//alert("pagingInfo.pagesCount:"+pagingInfo.pagesCount);
            ...
            _addCheckOptionCell: function(rowData, newRow) {
                ...
                if (p.checkOption == 'radio') {
                    ...
                    if (p.multiChooseMode == 5) {
                        createCheckbox(checkCell, setOptionDisabled, 'width:20px;border:2px;', '_chks', _id, 
                        function() {
                        ...
                        createCheckbox(checkCell, setOptionDisabled, 'width:20px;border:2px;', '_chks', _id, p.handleCheck, '', '', defalutChoose);
            ...
            _verifyAjaxAns: function(msg) {
               // eval(" tempData=" + msg);
				 ...
				 */
                var data;
                if (tempData.data) data= tempData.data;
                else data=tempData;
                var columnName = p['idColumn']; 
               // alert("columnName:"+columnName);
                if (typeof data[0] == 'undefined' || tempData.totalPage == 0) {
                    return false;
            ...
            isParent: function(rowobj) {
                if (!p.lazy) {
                	...
                	var isLeaf=rowobj[p.leafColumn];
                	if(isLeaf==0)
                		return '1';
                	else
                		return '0';
        ...
        lazypagey2 = 0;
		// lsq
        var lzpagetabletemp = ['<table cellspacing=0 cellpadding=0  class="lazyPageTable"><tr><td>', '<button class=" lazypagebarbtn firstPage" id="_fstbtn_$" title="第一页"/></td>', '<td><button class=" lazypagebarbtn prevPage"  title="前一页" id="_prebtn_$"/></td>', '<td><button class=" lazypagebarbtn nextPage" id="_nxtbtn_$"  title="后一页"/></td>', '<td><button class=" lazypagebarbtn lastPage" id="_lstbtn_$" title="最后一页"/></td>', '<td class="splitbar"><td  class="lazyPagingMsg" >总共<span id="_lazy_page_count_$" class="lazyPagingNum">', '</span>条 当前显示第<span id="_lazy_page_currentpage_$"	class="lazyPagingNum"></span>页</td></table>'].join('');
        ...
        if (p.pageBar) {
            var _panel_div = $("<div id='_panel_div' class='panelBar' height='35'></div>")[0];
        ...
        elct.innerHTML = '';
        // 添加等待条
        $('<div class="showprogress" id="showmsg"><div id="_centerMsg" class="centerMsg" align="top"><img src="' + p.lazyLoadImg + '">Loading...</div></div>').appendTo(elct);
    ...
    $.fn.debug = function(msg){
    	return this.each(function() {
    		//var g = this.gt;
    		//var tr = $("tr[id='_node"+dId+"']");
    		//if ($(tr).attr('_node_isparent')=='1'){
    			//var path = $(tr).attr('_node_path')+',_node'+dId;
    			//g._deleteByPath(path);
    		//}
    		if(this.gt)this.gt._d(msg);
    	 });
    ...
    // create a new child
    $.fn.addSingleChild = function(pid,data){
    	return this.each(function(){
    		if(this.gt) this.gt._addSingleChild(pid,data);
    	});
    };
    ...
    $.fn.getCheckedFileName = function(){
    	var tr;
    	$('[name=_chks]:checked').each(function() {
    		tr = $(this).closest('tr');
    	});
    	return $(tr).children().eq(0).text();
    }
    $.fn.change = function(dId,tdPro,data){
        return this.each(function() {
        	var g = this.gt;
        	var tr;
        	var value = data.split(',');
        	var content = $("tr[id='_node"+dId+"']").children("td[_td_pro='"+tdPro+"']").html();
        	content = content.replace(value[0],value[1]);
        	$("tr[id='_node"+dId+"']").children("td[_td_pro='"+tdPro+"']").html(content);
        	var img = $("img[id='img_"+dId+"']");
        	if($(img).attr('src').indexOf('minus.gif')>-1){
        		$(img).bind('click',function(e) {
        			g.closeChildrenTable('img_' + dId,null,e) ;
        		});
        	}else{
            	$(img).bind('click',function(e){
            		g.openChildrenTable('img_'+dId, null, e);
            	});
            }
